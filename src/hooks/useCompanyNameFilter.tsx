...
const isCard = (item: Company | UserCard): item is UserCard => {
  return (item as UserCard).company !== undefined; // Change this based on a unique property
};

type FilterInput = Company[] | UserCard[];
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      if (isCard(item)) {
        ...
        activityTypeId = item.company.activityTypeId; // Handle optional chaining
        type = 'card';
